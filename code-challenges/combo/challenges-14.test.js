...
const toTitleCase = (arr) => {
  return arr.map(element => element.charAt(0).toUpperCase() + element.substr(1))
};
...
let biggerThanLuke = (arr) => {
  return arr.filter(element => element.mass > 77 ).map(element => element.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  if(property === 'name'){
    return arr.sort( (a, b) => a.name.localeCompare(b.name) || a.price < b.price);
  } else{
