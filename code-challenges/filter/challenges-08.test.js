...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(index =>{
    return !forbiddenValues.includes(index);
  })
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(stats => stats.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(stats => stats.baseStat > minBaseStat).map( element => element.stat.name );
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter( objCharacters => {
    if(!objCharacters.children){
      return objCharacters.name;
    }
  });
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter( element => typeof element !== 'string' ).map( element => element % 2 === 0 ? 'even' : 'odd' );
