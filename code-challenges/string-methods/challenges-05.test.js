...
const howMuchPencil = (str) => {
  ...
  for(let i =0 ; i < str.length; i++){
    const sliceFirst = str.substring(i);
    result.push(sliceFirst);
  }
  result.push('');
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for(let i in recipe.ingredients){
    words.push(recipe.ingredients[i].split(' '));
  }

  for(let i in words){
    const word = words[i].slice(2, words[i].length).join(' ');
    result.push(word);
  }
  return result;
};
...
const stepActions = (recipe) => {
  ...
  for(let i in recipe.steps){
    let firstIndex = recipe.steps[i].split(' ')[0];
